...
packages = [
    "telebot","pyTelegramBotAPI","pymongo","requests","python-telegram-bot","psutil","flask","certifi ",
]
...
BOT_TOKEN = '7385947182:AAGn2debrMPkZGELbERTsGQe0d-spvS6e7g'
...
async def run_attack_command_on_codespace(target_ip, target_port, duration, chat_id):
    ...
    try:
        ...
        output = stdout.decode().replace("", "@ddos_wala")
        error = stderr.decode()
...
def send_owner_info(message):
    owner_message = "Owner:- https://t.me/Bgmi_owner_420"
...
def send_welcome(message):
    ...
    welcome_message = ("`/genkey` - (To generate key)"
...
def genkey_command(message):
    ...
    if not is_admin(user_id):
        bot.send_message(chat_id, "*You are not authorized to generate keys.\nContact Owner: @ddos_wala*", reply_markup=keyboard, parse_mode='Markdown')
        return
...
def attack_button_handler(message):
    ...
    if not found_user:
        bot.send_message(chat_id, "*You are not registered. Please redeem A key To Owner:- @ddos_wala*", parse_mode='Markdown')
        return

    valid_until = datetime.fromisoformat(found_user['valid_until'])
    if datetime.now() > valid_until:
        bot.send_message(chat_id, "*Your key has expired. Please redeem A key To Owner:- https://t.me/Bgmi_owner_420 ", parse_mode='Markdown')
        return
...
def my_account(message):
    ...
    if found_user:
        ...
        account_info = "Please redeem A key To Owner:- https://t.me/Bgmi_owner_420 "
